...
class AStarPlanner:
    ...
    def __init__(self, ox, oy, resolution, rr, fc_x, fc_y, tc_x, tc_y,minc_x,minc_y):
        """
            ...
            for i, _ in enumerate(self.motion): # tranverse the motion matrix
                ...
                if self.calc_grid_position(node.x, self.min_x) in self.tc_x:
                    if self.calc_grid_position(node.y, self.min_y) in self.tc_y:
                        ...
                        node.cost = node.cost + self.C_T*self.Delta_T_A * self.motion[i][2]
                ...
                if self.calc_grid_position(node.x, self.min_x) in self.fc_x:
                    if self.calc_grid_position(node.y, self.min_y) in self.fc_y:
                        ...
                        node.cost = node.cost + self.C_T*self.Delta_F_A * self.motion[i][2]
                   
                # add minus-consuming area
                if self.calc_grid_position(node.x, self.min_x) in self.minc_x:
                    if self.calc_grid_position(node.y, self.min_y) in self.minc_y:
                        # print("fuel consuming area!!")
...
def main():
    ...
    if show_animation:  # pragma: no cover
        ...
        plt.plot(minc_x, minc_y, "ob") # plot the minus cost consuming area

        plt.grid(True) # plot the grid to the plot panel
        plt.axis("equal") # set the same resolution for x and y axis 

    a_star = AStarPlanner(ox, oy, grid_size, robot_radius, fc_x, fc_y, tc_x, tc_y, minc_x, minc_y)
